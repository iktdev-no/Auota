...
export function addBackup(path: string) {
  return apiPostPlaintext<string>("/backup/add", path);
}
...
export function ignoreBackupOrFolder(data: BackupIEUpdate) {
  return apiPost<BackupIEUpdate, void>("/backup/exclude", data);
}

export function unignoreBackupOrFolder(data: BackupIEUpdate) {
  return apiPost<BackupIEUpdate, void>("/backup/include", data);
}
